...
import { useLazyGetAsistQuery, useCreateAsistMutation, useUpdateAsistMutation } from '../../services/apiAsistente';
import { useEffect } from 'react';
import { Link, useNavigate, useParams } from 'react-router-dom';
import toast from 'react-hot-toast';
import { useSelector } from 'react-redux';
import { auth_state } from '../../features/authSlice';
import DropLevel from './dropdownNivel';
export default function AsistenteForm() {
  const navigate = useNavigate();
  const { id } = useParams();
  const [getAsistenteById, { data, isLoading }] = useLazyGetAsistQuery()
  const [updateAsistente] = useUpdateAsistMutation()
  const [createAsistente] = useCreateAsistMutation()
  ...
  const handleSubmit = (values) => {
    ...
    if (id) {
      ...
      createAsistente({ ...rest, departamento: dep[1] })
        .unwrap()
        .then(() => {
          navigate('/asistentes');
  ...
  return (
    <>
      <header className='flex justify-between pb-10'>
        ...
        <h3 className='font-bold text-lg  text-center'>{id ? 'Actualizar' : 'Crear'} Asistente</h3>
      </header>
        ...
        {({ setFieldValue, touched, isValid, handleChange, handleBlur }) => {
          useEffect(() => {
            if (!isLoading && data) {
              ...
              setFieldValue("nivelEscolar", data.nivelEscolar);
          ...
          return (
            <Form>
              ...
              <DropLevel name='nivelEscolar' value={data?.nivelEscolar} label="Nivel Escolar" />
              ...
              <Btn type='submit' disabled={!isValid} label={`${id ? 'Actualizar' : 'Registrar'} Asistente`} />
