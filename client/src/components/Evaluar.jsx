...
import { useUpdateComercialMutation } from "../services/apiTable";

export default function Evaluar({setIsopen, isOpen, id}) {
    const [updateComercial] = useUpdateComercialMutation();
    const data = useSelector(state => state.colTable.data);
    const [selectEval, setSelectEval] = useState();
    const [nombre, setNombre] = useState()
    useEffect(() => { 
        const datos = data?.filter(item => item.id == id)[0]
        setSelectEval(datos?.evaluacion);
